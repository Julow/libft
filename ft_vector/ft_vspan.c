...
/*   Updated: 2017/01/12 12:12:58 by jaguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*ft_vspan(t_vector *v, t_vec2u span, void const *data, uint32_t n)
{
	...
	if ((v->length + move) > v->capacity)
		ft_vreserve(v,
			MAX(v->capacity * 2, MAX(VECTOR_MIN_CAP, v->length + move)));
	if (move != 0)
	{
		if (v->length > span.y)
			memmove(VECTOR_GET(*v, span.y + move), VECTOR_GET(*v, span.y),
				VECTOR_OFF(*v, v->length - span.y));
		v->length += move;
	}
	if (data != NULL)
		memcpy(VECTOR_GET(*v, span.x), data, VECTOR_OFF(*v, n));
